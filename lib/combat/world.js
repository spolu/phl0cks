...
var world = function(spec, my) {
  ...
  advance = function() {
    ...
    var process = function(a) {
      ...
      var pos = a.position();
      var vel = a.velocity();
      if(pos.x > config.HALFSIZE) {
        pos.x = config.HALFSIZE - Math.abs(pos.x - config.HALFSIZE);
        if(pos.x * vel.x > 0)
          vel.x = -vel.x
      }
      if(pos.x < -config.HALFSIZE) {
      ...
      if(pos.y > config.HALFSIZE) {
        pos.y = config.HALFSIZE - Math.abs(pos.y - config.HALFSIZE);
        if(pos.y * vel.y > 0)
          vel.y = -vel.y
      }
      if(pos.y < -config.HALFSIZE) {
