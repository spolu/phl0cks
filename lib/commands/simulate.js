...
var simulate = function(spec, my) {
  ...
  execute = function(args, cb_) {
    if(args.length < 3)
      return new cb_(new Error('<size> or <path1/2> missing'));
  ...
  help = function(args, cb_) {
    ...
    my.log.help('  argument and simulate a combat with <size> ships for each phl0ck. The simulation');
    ...
    my.log.help('--randomize'.bold + '   randomize the simulation (initial position/velocity)');
    my.log.help('');
    my.log.help('Exmaples:'.yellow);
