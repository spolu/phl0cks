...
var combat = function(spec, my) {
  ...
  init = function() {
    my.phl0cks.forEach(function(ph) {
      ...
      for(var i = 0; i < size; i ++) {
        ...
        // collide handler
        my.ships[id].sh.on('collide', function() {
          my.ships[id].destroy();
        });

        // shoot handler
        my.ships[id].on('shoot', function(d, s) {
          ...
          my.missiles[id].on('collide', function() {
  ...
  run = function(step_, control_, end_) {
    ...
    var control_phase = function() {
      ...
      // callback
      control_(my.world.step(), control);
    };
    ...
    while(1) {
      ...
      if(my.world.step() * config.STEP_TIME >= config.COMBAT_INTERVAL) {
        if(typeof end_ === 'function')
          end_(my.world.step(), my.world);
        break;
      }
      // control_ 
      if(my.world.step() - my.last_control >= config.CONTROL_INTERVAL) {
        control_phase();
      }
      // step_
